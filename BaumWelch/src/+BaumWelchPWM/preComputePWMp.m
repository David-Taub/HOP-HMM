...
function pcPWMp = preComputePWMp(Xs, params)
    [PWMs, params.lengths] = BaumWelchPWM.PWMs();
    % [~, n, J] = size(PWMs);
    % [N, ~] = size(Xs);
    %0 padding for handling the edges probability calculation
    Xs1H = cat(2, matUtils.mat23Dmat(Xs, params.n), zeros(params.N, params.J, params.n));
    PWMsRep = permute(repmat(PWMs, [1, 1, 1, params.N]), [4, 3, 2, 1]);
    pcPWMp = preComputePWMpAux(PWMsRep, Xs1H, params);
end
% calculating the probability of each position in the sequences
% to be emitted by each PWM
function pcPWMp = preComputePWMpAux(PWMsRep, Xs1H, params)
    % pcPWMp - N x k x L-1
    % [N, J, ~, k] = size(PWMsRep);
    % [~, L, ~] = size(Xs1H);
    % L = L - J;
    ...
    try
        ...
        mask = repmat(1:params.J, [params.N, 1, params.k]) > repmat(permute(params.lengths, [1,3,2]), [params.N,params.J,1]);
        pcPWMp = zeros(params.N, params.k, params.L);
        for t = 1:params.L
            pcPWMp(:, :, t) = BaumWelchPWM.getPWMp(params, PWMsRep, Xs1H, t, mask);
        end
        pcPWMp = cat(3, zeros(params.N, params.k, params.J), pcPWMp);
        fprintf('\n');
